...
# Cálculo de los valores estandarizados de los límites
z_inf <- (lim_inf - media) / s
z_sup <- (lim_sup - media) / s

# Cálculo de la probabilidad utilizando pnorm
prob <- pnorm(z_sup) - pnorm(z_inf)
...
```{r}
# Especificamos los parámetros del problema
mu <- 20 # media
sigma <- 3 # desviación estándar
n <- 15 # tamaño de la muestra
varianza_limite <- 12 # límite de varianza

# Calculamos la probabilidad utilizando la distribución chi-cuadrado
df <- n - 1 # grados de libertad
chi_cuadrado <- (n - 1) * varianza_limite / (sigma^2) 
probabilidad <- pchisq(q = chi_cuadrado, df = df, lower.tail = FALSE)

# Imprimimos el resultado
cat("La probabilidad de que la varianza de la muestra exceda los", varianza_limite, "segundos es:", probabilidad, "\n")
```
